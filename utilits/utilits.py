...
def prepare_data(path: str) -> dict:
    """
    Load and preprocess the SNLI data.
    Args:
    ...
    train_data = load_jsonl_data(os.path.join(path, "snli_1.0_train.json"))
    test_data = load_jsonl_data(os.path.join(path, "snli_1.0_test.json"))
    ...
    for data, data_name in zip([train_data, test_data], ["train", "test"]):
        ...
        labels = [d['gold_label'] for d in data]

        snli_dataset[data_name] = {"sentence1": sentences1, "sentence2": sentences2, "labels": labels}

    return snli_dataset
    
def format_dataset(path, split):
    """
    Formats the SNLI dataset for a specified split (e.g., "train", "test").
    Args:
        path (str): Path to the dataset.
        split (str): The split of the dataset to format.
    Returns:
        list: List of dictionaries, where each dictionary contains 'sentence1', 'sentence2', and 'label'.
    """
    snli_data = prepare_data(path)

    # Initialize data
    data = []

    # Create a list of dictionaries
    for s1, s2, label in zip(snli_data[split]["sentence1"], snli_data[split]["sentence2"], snli_data[split]["labels"]):
...
def create_dataset_of_label_propagation(text_data, pred_labels):
    ...
    """
    return Dataset.from_dict({"sentences": text_data, "labels": pred_labels})
